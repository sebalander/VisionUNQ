...
#        self.btn_Up.clicked.connect(self.ptzCam.moveUp)
    ...
    def open_stream(self):
        if self.cameraManager.ptzCam != None:
            self.showPTZStreaming()
        if self.cameraManager.feCam != None:
            self.showFEStreaming()
...
#                print 'Aca llamo a la camara pa que se mueva'
    ...
    def showFEStreaming(self):
        feUrl = self.cameraManager.feCam.getStreamUri() 
        logging.info("Abriendo captura desde " + feUrl)
        self.cameraManager.feCam.capture = cv2.VideoCapture(feUrl)
        
        while (self.cameraManager.feCam.capture.isOpened()):
            ret, frame = self.cameraManager.feCam.capture.read()
        ...
        if self.cameraManager.feCam.capture.isOpened()==False:
            print('No se pudo abrir la interfaz!')
